...
void TextIndexPostingsRankCursor::loadSegment(size_t segment_idx)
{
    ...
    const UInt64 max_blocks = (doc_count + IPostingListBlockCodec::BLOCK_SIZE - 1) / IPostingListBlockCodec::BLOCK_SIZE;
...
void TextIndexPostingsRankCursor::decodeBlock(size_t block_idx)
{
    ...
    const size_t count = (block_idx + 1 == block_count)
        ? segment.doc_count - (block_count - 1) * IPostingListBlockCodec::BLOCK_SIZE
        : IPostingListBlockCodec::BLOCK_SIZE;
    ...
    decoded_count = count;
    index_in_block = 0;
    block_first_rank = static_cast<UInt64>(block_idx) * IPostingListBlockCodec::BLOCK_SIZE;
}
